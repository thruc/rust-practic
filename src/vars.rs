...
pub fn run() {
    ...
    // let i1 = 3; // unused variable: `f1`
    // let f1 = 0.1;

    // _をつけることでunused variableのwarningを消せる
    let _i1 = 3;
    let _f1 = 0.1;

    println!("{}", usize::BITS);
    println!("Memory address of const is: {:p}", &MAX_POINTS); // &をつけることで番地を値を取得できる

    let i2: i64 = 1;
    let i3: i64 = 2;
    // スタックの領域の８バイト分i3が後ろかどうか確認
    println!("Stack address of i2 is: {:p}", &i2);
    println!("Stack address of i3 is: {:p}", &i3);

    let y = 5;
    println!("Stack address of y is: {:p}", &y);
    // シャドーイングされる(スタック領域は別の番地に格納される)
    let y = y + 1;
    println!("Stack address of y is: {:p}", &y);
    let y = y * 2;
    println!("Stack address of y is: {:p}", &y);
